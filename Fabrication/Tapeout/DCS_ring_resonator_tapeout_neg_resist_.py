...
def main(args):
    ...
    length_between = args.length_between
    ...
    c = gf.Component("DCS_ring_tapeout_with_taper_neg_resist")
    ...
    for i in range(0, steps+1):
        ...
        g = c << gf.components.straight(length=length_between,width=ring_wg_width) # waveguides between
        h = c << gf.components.straight(length=length_between,width=ring_wg_width)

        input_coupler1 = c << gf.components.taper(width1=ring_wg_width, width2=output_mode_size, length=300)
        input_coupler2 = c << gf.components.taper(width1=ring_wg_width, width2=output_mode_size, length=300)
        output_coupler1 = c << gf.components.taper(width1=ring_wg_width, width2=output_mode_size, length=300)
        output_coupler2 = c << gf.components.taper(width1=ring_wg_width, width2=output_mode_size, length=300)
       
        ## Copy the previous input coupler and move it to the output coupler location

        # output_coupler = c << gf.components.taper(width1=ring_wg_width, width2=output_mode_size, length=300)  
        ...
        ring0_gaps.append(ring_gap-(iteration_width/2))
        ring0.center = ([length1/2,total_y+(50*i)]) ## This ensure all the waveguide are 1000nm apart

        
        g.connect("o1", destination=ring["o1"])
        h.connect("o1", destination=ring["o4"])

        ring0.connect("o4",destination=g["o2"])


        # I'm not sure why code needed to be like this to be at the appropriate ports
        b.connect("o2", destination=ring0["o3"]) # left bottom WG 
        d.connect("o2", destination=ring0["o2"]) # left top WG
        e.connect("o1", destination=ring["o2"]) # right bottom WG
        f.connect("o1", destination=ring["o3"]) # right top WG
 
        input_coupler1.connect("o1", destination=b["o1"])
        input_coupler2.connect("o1", destination=d["o1"])
        output_coupler1.connect("o1", destination=f["o2"])
        output_coupler2.connect("o1", destination=e["o2"])

        d_ref1 = out.add_ref(c)  # Reference the Component "c" that 3 references in it
        d_ref2 = out << c

        d_ref2.dmove([300, 200])
    ...
    gdspath = c.write_gds("DCS_ring_tapeout_neg_resist.gds", precision=1e-9, unit=1e-6,with_metadata=True)
